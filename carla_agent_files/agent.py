...
from agent_utils.filter_functions import *
from agent_utils.coordinate_utils import preprocess_compass, inverse_conversion_2d
from agent_utils.explainability_utils import *

from data_agent_boxes import DataAgent
# from training.PlanT.dataset import generate_batch, split_large_BB
# from training.PlanT.lit_module import LitHFLM
from model.PlanT.dataset import generate_batch,split_large_BB
...
carla_maps_dir='/home/masterthesis/pos_lane/carla_maps'
with open(join(carla_maps_dir, 'carla_maps.pickle'), 'rb') as handle:
            maps = pickle.load(handle)
            # transpose all arrays
            carla_maps = {}
            for town_name, lane_list in maps.items():
                new_lane_list = []
                ...
                carla_maps[town_name] = new_lane_list

with open(join(carla_maps_dir, 'carla_meta.pickle'), 'rb') as handle:
    carla_meta = pickle.load(handle)
...
class PlanTAgent(DataAgent):
    ...
    def _init(self, hd_map):
        ...
        self.town_name=self._world.get_map().name
        
        # ego_location=self._vehicle.get_location()
        ego_transform = self._vehicle.get_transform()
        ego_rotation = ego_transform.rotation
        self.ego_yaw =  ego_rotation.yaw/180*np.pi
    ...
    def _get_control(self, label_raw, input_data):
        ...
        x, y, _, tp, light,_,_ = input_batch
        _, _, pred_wp, attn_map,_,_ = self.net(x, y, target_point=tp, light_hazard=light)
    ...
    def get_input_batch(self, label_raw, input_data):
        ...
        sample = {'input': [], 'output': [], 'brake': [], 'waypoints': [], 'target_point': [], 'light': [],'label_lane':[],'label_junction':[]}
        ...
        ego_loc = np.array([input_data['gps'][0], -input_data['gps'][1]])
        # ego_ori = -input_data['theta']
        ego_ori=-self.ego_yaw
    

        step_x = carla_meta[self.town_name]['step_x']
        step_y = carla_meta[self.town_name]['step_y']
        idx_x = int((ego_loc[0] - carla_meta[self.town_name]['map_min_x'] + 0.5*step_x) // step_x)
        idx_y = int((ego_loc[1] - carla_meta[self.town_name]['map_min_y'] + 0.5*step_y) // step_y)
        lane_ids = []
        if (idx_x, idx_y) in carla_meta[self.town_name]['lane_grid']:
            lane_ids = carla_meta[self.town_name]['lane_grid'][idx_x, idx_y]
        lane_list = np.array([carla_maps[self.town_name][lane_id] for lane_id in lane_ids])

        
        lane_map,lane_yaw,lane_length=self.get_lane_map(lane_list,ego_loc,ego_ori)

        data_lane = [
                    [
                        3.0,  # type indicator for lane
                        float(x[1]) - float(label_raw[0]["position"][0]) ,
                        -float(x[0]) - float(label_raw[0]["position"][1]) ,
                        float(y) * 180 / 3.14159265359,
                        float(j),
                        0.5,
                        z,
                    
                    ]
                    for j, (x,y,z) in enumerate(zip(lane_map,lane_yaw,lane_length))
                 

                ] 
        
        features = data_car + data_route +data_lane
    ...
    def get_lane_map(self, lane_list, ego_loc, ego_ori):
        # render map
        ...
        for lane in lane_list:
            ...
            # if len(lane_center)<5:
            #     continue
            # elif 5<=len(lane_center)<=10:
            #     center=lane_center[0]
            #     center_1=lane_center[-1]
            #     theta=np.arctan2((center_1-center)[0],(center_1-center)[1])
            #     theta=self.normalize_angle(theta+ego_ori-np.pi/2)
            #     center=(center-ego_loc)@R_ego
            #     lane_map.append(center)
            #     lane_yaw.append(theta)
            #     lane_length.append(len(lane_center))
                
            # else:
            #     idx=np.linspace(0,len(lane_center)-1,round(len(lane_center)/10)+1)
            #     for i in range(len(idx)-1):
            #         center=lane_center[int(idx[i])]
            #         center_1=lane_center[int(idx[i+1])]
            #         theta=np.arctan2((center_1-center)[0],(center_1-center)[1])
            #         theta=self.normalize_angle(theta+ego_ori-np.pi/2)
            #         center = (center - ego_loc) @ R_ego
            #         if -60<np.linalg.norm(center)<60:
            #             lane_map.append(center)
            #             lane_yaw.append(theta)
            #             lane_length.append(15)
            
            lane_left=lane['left_boundary']
            lane_right=lane['right_boundary']
            # transform to ego coordinate system
            if len(lane_left)<5:
                continue
            elif 5<=len(lane_left)<=15:
                left=lane_left[0]
                left_1=lane_left[-1]
                
                right=lane_right[0]
                right_1=lane_right[-1]
                
                theta_right=np.arctan2((right_1-right)[0],(right_1-right)[1])
                theta_right=self.normalize_angle(theta_right+ego_ori-np.pi/2)
                right=(right-ego_loc)@R_ego
                
                theta_left=np.arctan2((left_1-left)[0],(left_1-left)[1])
                theta_left=self.normalize_angle(theta_left+ego_ori-np.pi/2)
                left=(left-ego_loc)@R_ego
                
                if -60<np.linalg.norm(right)<60:
                    lane_map.append(right)
                    lane_yaw.append(theta_right)
                    lane_length.append(len(lane_right))
                if -60<np.linalg.norm(left)<60:
                    lane_map.append(left)
                    lane_yaw.append(theta_left)
                    lane_length.append(len(lane_left))
                
            else:
                idx=np.linspace(0,len(lane_left)-1,round(len(lane_left)/15)+1)
                for i in range(len(idx)-1):
                    left=lane_left[int(idx[i])]
                    left_1=lane_left[int(idx[i+1])]
                    
                    right=lane_right[int(idx[i])]
                    right_1=lane_right[int(idx[i+1])]
                    
                    theta=np.arctan2((left_1-left)[0],(left_1-left)[1])
                    theta=self.normalize_angle(theta+ego_ori-np.pi/2)
                    ...
                    if -60<np.linalg.norm(right)<60:
                        ...
                        lane_map.append(right)
                        lane_yaw.append(theta_right)
                        lane_length.append(10)
            if len(lane_map)>=100:
                break
  
        return lane_map,lane_yaw,lane_length
