...
class HFLM(nn.Module):
    def __init__(self, config_net, config_all):
        ...
        self.object_types = 3 + 1  # vehicles, route,lane +1 for padding and wp embedding
        ...
        if self.config_all.model.pre_training.bezier:
            
            self.bezier_order =  self.config_all.model.pre_training.bezier_order # order of the predicted bezier curve
            sample_times_normalized = torch.linspace(0, 1, self.config_all.model.training.pred_len + 1)[1:]
            mixing_mat = self.mixing_matrix(self.bezier_order, sample_times_normalized)
            self.register_buffer("mixing_mat", mixing_mat)
    ...
    def bernstein_matrix(self,order):
        mat = torch.zeros((order + 1, order + 1))
        for i in range(order + 1):
            for j in range(i + 1):
                mat[i, j] = binom(order, i) * binom(i, j) * ((-1) ** (i + j))
        return mat


    def mixing_matrix(self,bezier_order, sample_times):
        bernstein_mat = self.bernstein_matrix(bezier_order)

        times_mat = [torch.ones(len(sample_times))]
        for _ in range(bezier_order):
            times_mat.append(times_mat[-1] * sample_times)
        times_mat = torch.stack(times_mat, dim=-1)

        mixing_mat = times_mat @ bernstein_mat
        return mixing_mat


    def sample_bezier_curve(self,control_points, mixing_mat):
        sample_points = mixing_mat @ control_points
    ...
    def forward(self, idx, target=None, target_point=None, light_hazard=None):
        ...
        lane_mask = (input_batch_type == 3).unsqueeze(-1)
        other_mask = torch.logical_and(route_mask.logical_not(), torch.logical_and(car_mask.logical_not(),lane_mask.logical_not()))
        masks = [car_mask, route_mask, lane_mask,other_mask]
        # car_mask = (input_batch_type == 1).unsqueeze(-1)
        # route_mask = (input_batch_type == 2).unsqueeze(-1)
        # other_mask = torch.logical_and(car_mask.logical_not(),route_mask.logical_not())
        ...
        #same lane
        x_class=torch.zeros_like(x)
        for i in range(len(x_class)):
            x_class[i]=car_mask[i]*x[i]
        x_lane=[self.mlp_lane(item) for item in x_class] 
        x_junc=[self.mlp_junc(item) for item in x_class]
        ...
        #if bezier curve predict control points
        if self.config_all.model.pre_training.bezier:
            for _ in range(self.config_all.model.pre_training.bezier_order):
            ...
            pred_wp_gru = torch.stack(output_wp, dim=1)
            
            control_point1 = torch.zeros(size=(z.shape[0],1,2), dtype=z.dtype, device=z.device)
        
            control_points = torch.cat([control_point1, pred_wp_gru], dim=1)  # Bx(O+1)x2
            
            pred_wp = self.sample_bezier_curve(control_points, self.mixing_mat)  # BxTx2
        else:
        ...
        if (
            ...
            return logits, targets, pred_wp, attn_map, x_lane, x_junc
...
class Loss(nn.Module):
    ...
    def forward(self,output,label):
        ...
        f1=F1Score(task='multiclass',num_classes=self.num_classes).to(output[0].device)
        confmat=ConfusionMatrix(task='multiclass',num_classes=self.num_classes).to(output[0].device)
        score=0
        metrics=torch.zeros(self.num_classes,self.num_classes,dtype=torch.long).to(output[0].device)
        
        for i,item in enumerate(output):
            # y=torch.zeros(len(item),dtype=torch.long).to(item.device)+4
            y=torch.zeros(len(label[i])+1,dtype=torch.long).to(item.device)
            y[1:]=label[i]
            
            item=item[:len(label[i])+1]
            
            loss=F.cross_entropy(item,y)
            loss_cls+=loss
            
            _,pred=torch.max(item.data,dim=1)
            
            score+=f1(pred,y)
            
            metrics+=confmat(pred,y)
            
        loss_cls=loss_cls/len(output)
        
        acc=score/len(output)
        
        return loss_cls,acc,metrics
